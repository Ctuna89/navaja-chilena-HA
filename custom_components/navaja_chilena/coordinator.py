...
class NavajaCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        ...
        # Parse stop ids (from options or data)
        ...
        async def fetch_json(url: str) -> Any:
            try:
                            headers = {
                'User-Agent': 'HomeAssistant-NavajaChilena/2025.1 (+https://github.com/duvob90/navaja_chilena)',
                'Accept': 'application/json, text/plain, */*',
                'Referer': 'https://www.metro.cl/' if 'metro.cl' in url else None,
            }
            # Drop None values
        ...
        # Attempt to extract affected stations/incidents (very defensive)
...
if isinstance(metro_json, dict):
    ...
    if isinstance(lines, list):
        for ln in lines:
            ...
            # common keys candidates for incidents/stations
            affected = []
            details = []
            ...
            for key in ("estaciones", "stations"):
                val = ln.get(key)
                if isinstance(val, list):
                    # include stations with issues if marked with state != normal
                    for st in val:
            ...
            if affected or details:
                metro_details[lid] = {"affected_stations": sorted(set(affected)), "details": details}

        return {
            "usd": usd_val,
            "uf": uf_val,
            "metro_lines": metro_lines,   # per-line status
